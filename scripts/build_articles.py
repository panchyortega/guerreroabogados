...
def markdown_to_html(text):
    """Convert markdown to HTML. Supports ## headings, paragraphs, - lists."""
    if not text:
        return ""
    html = []
    lines = text.replace("\r\n", "\n").replace("\r", "\n").split("\n")
    ...
    for line in lines:
        ...
        if line_s.startswith("## "):
            if in_list:
                html.append("</ul>")
                in_list = False
            html.append(f"<h2>{line_s[3:].strip()}</h2>")
        elif line_s.startswith("# "):
            if in_list:
                html.append("</ul>")
                in_list = False
            html.append(f"<h2>{line_s[2:].strip()}</h2>")
        elif line_s.startswith(("- ", "* ", "• ")):
            if not in_list:
                html.append("<ul>")
                in_list = True
            html.append(f"<li>{line_s[2:].strip()}</li>")
        else:
            if in_list:
                html.append("</ul>")
                in_list = False
            html.append(f"<p>{line_s}</p>")
    if in_list:
        html.append("</ul>")
